...
public class CombineHistogram2 extends Histogram {

  public CombineHistogram2() {
    super();
  }

  public CombineHistogram2(HistogramProto proto) {
    super(proto);
  }
  ...
  public Double estimateFrequency(Double from, Double to) {
    ...
    Double pod1 = 0.0, pod2 = 0.0; // Partially overlap distances
    
    for (HistogramBucket bucket : buckets1) {
      Double dis = computeOverlapDistance(from, to, bucket);
      if(dis < (bucket.getMax() - bucket.getMin())) {
	pod1 += dis;
      }
    }
    ...
    double cc1 = 1.0 - (pod1 / (to - from));
    double cc2 = 1.0 - (pod2 / (to - from));
